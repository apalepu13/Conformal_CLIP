...
def main(args):
    ...
    exp_path = utils.getExperiment(args, mod_path)
    start, je_model, params, optimizer, best_val_loss = utils.startExperiment(args, exp_path)
    print(exp_path)
    filts = 'frontal,impression'
    ...
    mimic_dat = MedDataHelpers.getDatasets(source='m', subset = subset, augs = 1, filters = filters, train_mode=True)
    ...
    for epoch in range(start, args.num_epochs):
        je_model.train()
        tmimic = time.time()
        train_loss, train_losses = utils.train(train_data_loader_mimic, je_model, args, epoch, optimizer, total_step_mimic)
        ...
        if epoch % args.val_step == 0:
            ...
            je_model.eval()
            tval = time.time()
            val_loss, val_losses = utils.validate(val_data_loader_mimic, je_model, args)
            if not args.debug:
